...
class XianyuProductPublisher:
    ...
    def _parse_cookies(self, cookies_str: str) -> List[Dict[str, Any]]:
        ...
        支持三种格式:
        1. Fernet加密格式 (enc$开头) → 先解密
        2. Netscape格式 (# Netscape HTTP Cookie File) → 标准Netscape解析
        3. 简化格式 (key=value; key2=value2) → 原有逻辑
        ...
        # 如果是 Fernet 加密格式，先解密
        if cookies_str.startswith('enc$'):
            try:
                from cryptography.fernet import Fernet
                import os
                key_path = os.path.join(os.path.dirname(__file__), 'data', '.secret_encryption.key')
                if os.path.exists(key_path):
                    with open(key_path, 'rb') as f:
                        fernet = Fernet(f.read())
                    cookies_str = fernet.decrypt(cookies_str[4:].encode()).decode()
            except Exception:
                pass
            ...
            for line in cookies_str.strip().split('\n'):
                ...
                if len(parts) >= 7:
                    ...
                    if not name or not value:
                        continue
                    for cookie_domain in ['.goofish.com', '.taobao.com']:
                        cookies.append({
                            'name': name.strip(),
                            'value': value.strip(),
    ...
    async def _fill_product_info(self, product: ProductInfo) -> bool:
        ...
        try:
            ...
            # 如果页面存在标题输入框，则优先填写，找不到时忽略
            if product.title:
                title_input = await self._find_element_with_fallback('title_input', timeout=2000)
                if title_input:
                    await self._simulate_mouse_movement(title_input)
                    await title_input.click()
                    await title_input.fill('')
                    await title_input.fill(product.title)
                    await self._random_delay()
    ...
    async def _verify_publish_success(self) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            left_publish_page = self.PUBLISH_URL not in current_url and 'publish' not in current_url.lower()
            ...
            for selector in detail_page_selectors:
                try:
                    element = await self.page.query_selector(selector)
                    if element and await element.is_visible():
                        if not left_publish_page:
                            logger.warning(f"【{self.cookie_id}】在发布页中检测到详情元素样式，继续等待更明确的成功信号: {selector}")
                            continue
            ...
            # 方法4: 检查是否有成功提示（仅作为详情页确认的辅助信号）
            success_msg = await self._find_element_with_fallback('success_message', timeout=3000)
            if success_msg:
                logger.info(f"【{self.cookie_id}】发布成功提示已显示")

            if left_publish_page:
                logger.warning(f"【{self.cookie_id}】页面已离开发布页，但未检测到可靠的商品详情信号，当前 URL: {current_url}")
            else:
    ...
    async def batch_publish(self, products: List[ProductInfo]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, product in enumerate(products):
                ...
                success, product_id, product_url = await self.publish_product(product)
